...
call_price_table, delta_table = bs_parameter(s.VOL, ttm / s.T, price_table, s.K,s.R, s.DIVIDEND)
print("\n Asset path simulation and BS parameter calculation are completed")

if s.TRAINING_SAMPLE <= 100000:
...
for i in tqdm(range(len(price_table))):
    ...
    for t in range(s.M):
        ...
        if t == s.M-1:
            transaction_cost = (asset_price[t]*abs(new_position - position)*s.TRADING_COST + new_position*asset_price[t+1]*s.TRADING_COST)
        else:
            transaction_cost = (asset_price[t]*abs(new_position - position)*s.TRADING_COST)
        
        # Reward is calculated as expected hedging cost + constant * variance of total pnl from hedging.
        ...
        reward = transaction_cost + s.constant * \
        ( transaction_cost-(asset_price[t+1] - asset_price[t])*new_position + \
    ...
    for t in range(s.M):
        ...
        q[t][price_state[t]][lookup_position][action_list[t]] = q[t][price_state[t]][lookup_position][action_list[t]] + s.ALPHA * (qnew - q[t][price_state[t]][lookup_position][action_list[t]]) 
