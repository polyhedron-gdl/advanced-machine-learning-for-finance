...
from Lib.config import get_input_parms, loadConfig
from Lib.finder import find_pos
# -----------------------------------------------------
...
class discount_curve:
    ...
    def annuity(self, ts=0., Dt=1., p=1):
        A = 0
        for n in range(p):
            A += Dt*self.P_0t(ts + (n+1)*Dt)
        return A
    # ----------------------------------------
            
    def swap_rate(self, ts=0., Dt=1., p=1):
        A = self.annuity(ts= ts, Dt=Dt, p=p)
        return ( self.P_0t(ts) - self.P_0t(ts+p*Dt) )/A, A
    # -----------------------------------------------------------------

    def fw_rate(self, ts=0., tm=1.):
        return ( self.P_0t(ts)/self.P_0t(tm) - 1. )/(tm - ts)
    # -----------------------------------------------------------------

    def cap_parity_rate(self, ts=0., Dt=1., p=1):
        A = self.annuity(ts= ts, Dt=Dt, p=p)
        R = 0.0
        for n in range(p):
            ts = n*Dt
            tm = ts + Dt
            R += Dt*self.P_0t(tm)* self.fw_rate( ts, tm)
        return R/A, A
    # -----------------------------------------------------------------
